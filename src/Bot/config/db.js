...
module.exports = class DB {
    ...
    constructor(database = false){

        this.path_databases = __dirname + '../Databases/'

        if(database){
            this.db = new sqlite3.Database(`${this.path_databases}${database}.sqlite3`)
        }
    }

    connect(name){
        console.log(`tentando conectar ao DB ${name}`);
        return new sqlite3.Database(`${this.path_databases}${name}.sqlite3`, erro => {
